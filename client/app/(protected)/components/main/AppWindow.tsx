...
export default function AppWindow({
  ...
  function moveDrag(event: PointerEvent<HTMLElement>) {
    ...
    const maxX = Math.max(0, (window.innerWidth - bounds.width) / 2 - 12);
    const maxY = Math.max(0, (window.innerHeight + 900 - bounds.height) / 2);
    setPosition({
      x: Math.max(-maxX, Math.min(maxX, start.left + event.clientX - start.x)),
      y: Math.max(-maxY, Math.min(maxY, start.top + event.clientY - start.y)),
    });
  }
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 top-0 bottom-12 z-30 flex items-center justify-center p-0">
      <div
        ref={frame}
        role="dialog"
        aria-labelledby={titleId}
        className={`pointer-events-auto flex flex-col overflow-hidden border border-white/15 bg-[#11151e] text-white ${maximized ? "absolute top-0 h-full w-full rounded-none" : "rounded-xl shadow-sm"}`}
        style={
          maximized
            ? undefined
            : {
